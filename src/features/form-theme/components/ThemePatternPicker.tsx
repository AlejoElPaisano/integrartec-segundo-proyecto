...
import { PATTERN_OPTIONS } from "@/features/form-theme/utils";
...
export function ThemePatternPicker() {
  ...
  return (
    <div role="radiogroup" aria-label="Patrón de fondo" className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        return (
          ...
          >
            <div
              className={cn(
                "absolute inset-0",
                option.value === "dots" && "form-pattern-dots",
                option.value === "grid" && "form-pattern-grid",
                option.value === "waves" && "form-pattern-waves"
              )}
              style={{
            ...
            />
            <span className="absolute inset-x-0 bottom-1 text-center text-xs font-medium text-text">
              {option.label}
            </span>
            {isSelected && (
